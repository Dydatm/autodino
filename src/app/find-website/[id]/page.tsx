...
export default function FindWebsitePage() {
  ...
  const fetchUserAndJob = async () => {
    try {
      ...
      if (jobResponse.ok) {
        ...
        setJob(jobInfo)
        setJobName(jobInfo.name || 'Find Website')
  ...
  const handleConfigure = async () => {
    if (!companyColumn || !addressColumn) { setError('Veuillez sélectionner les deux colonnes'); return }
    ...
    try {
      ...
      const res = await fetch('/api/configure', {
        ...
        body: JSON.stringify({ jobId, name: jobName, companyColumn, addressColumn }),
      })
      const data = await res.json()
      ...
      <main className="max-w-7xl mx-auto p-6">
          ...
          <section className="bg-white rounded-2xl shadow p-8">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-3">
                ...
                <button onClick={()=>setIsEditingName(true)} className="p-2 rounded-md hover:bg-black/5 transition" title="Renommer">✏️</button>
              </div>
            </div>
            <h2 className="text-lg font-semibold mb-1">Étape 1 — Configuration</h2>
            <p className="text-sm text-gray-500 mb-4">Nomme, dépose ton CSV/XLSX, choisis les colonnes Entreprise et Adresse, puis termine la configuration.</p>
            <div className="space-y-5">
              ...
              {columns.length>0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm mb-1">Colonne Entreprise</label>
                    <select value={companyColumn} onChange={(e)=>setCompanyColumn(e.target.value)} className="w-full border rounded-lg px-3 py-2">
                      <option value="">Sélectionner…</option>
                      {columns.map((c)=> (<option key={c} value={c}>{c}</option>))}
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm mb-1">Colonne Adresse</label>
                    <select value={addressColumn} onChange={(e)=>setAddressColumn(e.target.value)} className="w-full border rounded-lg px-3 py-2">
                      <option value="">Sélectionner…</option>
                      {columns.map((c)=> (<option key={c} value={c}>{c}</option>))}
                    </select>
                  </div>
              ...
              <div className="flex gap-3">
                <button onClick={handleConfigure} disabled={!companyColumn || !addressColumn || isConfiguring} className="px-4 py-2 rounded-lg text-white disabled:opacity-50 transition-colors" style={{ backgroundColor: '#37A646' }}>{isConfiguring ? 'Configuration...' : 'Terminer la configuration'}</button>
              </div>
            </div>
          </section>
        )}
        {showStep2 && (
          <section className="bg-white rounded-2xl shadow p-8">
            ...
            <p className="text-sm text-gray-500 mb-4">Lance et télécharge le résultat.</p>
